...
function vocabularyDatabase(system){
  ...
  const groupHTML=[...groups.entries()].map(([key,records])=>{const [topic,title]=key.split('|');return `<section class="vocab-topic-group" data-vocab-group><div class="vocab-group-heading"><span>${escapeHTML(topic)}</span><h3>${escapeHTML(title)}</h3><small>${records.length} terms</small></div><div class="master-vocab-list">${records.map(record=>`<button class="master-vocab-term ${record.priority==='Honors Extension'?'honors':''}" type="button" data-vocab-term="${escapeHTML(record.term)}" data-vocab-priority="${escapeHTML(record.priority)}" data-vocab-topic="${escapeHTML(record.topic)}" data-vocab-title="${escapeHTML(record.topicTitle)}" data-vocab-connection="${escapeHTML(record.connection)}">${escapeHTML(record.term)}${record.priority==='Honors Extension'?'<sup>H</sup>':''}</button>`).join('')}</div></section>`}).join('');
  return `<section class="vocab-database" id="vocabulary-unit-${system.unit}"><div class="vocab-database-titlebar"><span>VOCAB_DATABASE.DAT</span><span>UNIT_${String(system.unit).padStart(2,'0')}</span></div><div class="vocab-database-body"><div class="vocab-database-heading"><div><p class="eyebrow">Clickable master list</p><h2>Unit vocabulary</h2><p>Choose any term to see its priority, topic, course connection, and mastery expectations.</p></div><label class="vocab-filter-label">Find a term<input class="vocab-filter" type="search" placeholder="Search this unit…" data-vocab-filter></label></div><div class="vocab-priority-key"><span><i></i> Core</span><span><i class="honors"></i> Honors extension</span></div><div class="vocab-groups">${groupHTML}</div></div></section>`;
}
...
main.addEventListener('click',event=>{
  ...
  document.querySelector('#vocab-priority').className=button.dataset.vocabPriority==='Honors Extension'?'honors':'core';
