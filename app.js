...
function renderToday(){
  ...
  host.innerHTML=`<div class="home-toolbar"><div><p class="eyebrow">Current course plan</p><h1>Today</h1></div><div><a class="button" href="#calendar">Full daily navigator →</a><a class="button secondary" href="#systems">Body systems</a></div></div><div class="today-card"><div class="today-date"><span class="dow">${formatDate(d.date,{weekday:'long'})}</span><span class="daynum">${formatDate(d.date,{day:'numeric'})}</span><span class="month">${formatDate(d.date,{month:'long'})}</span></div><div class="today-content"><div class="status-row"><span class="topic-badge">${topic?`Topic ${escapeHTML(topic.code)}`:'Daily Plan'}</span><span class="eyebrow">Week ${escapeHTML(d.week)}</span></div><h2>${escapeHTML(topic?.title||'Course work')}</h2><p class="target">${escapeHTML(topic?.target||d.topicText||'Review the plan below.')}</p><div class="today-grid"><div><span>In class</span><p>${escapeHTML(d.classwork||'See classroom instructions.')}</p></div><div><span>Biozone</span><p>${escapeHTML(d.biozone||'No pages assigned.')}</p></div><div><span>After class</span><p>${escapeHTML(d.home||'No homework posted.')}</p></div></div></div></div>`;
}
...
function dayCard(d){const t=topicByCode(d.topic);const today=d.date===new Date().toISOString().slice(0,10);return `<article class="day-card ${today?'current':''}"><div class="date-line"><span class="weekday">${escapeHTML(d.day||formatDate(d.date,{weekday:'short'}))}</span><span class="date-small">${formatDate(d.date)}</span></div>${t?`<span class="topic-badge">${escapeHTML(t.code)}</span><h3>${escapeHTML(t.title)}</h3><p class="mini-target">${escapeHTML(t.target)}</p>`:'<h3>Course work</h3>'}<div class="day-block"><b>Class</b><p>${escapeHTML(d.classwork||'—')}</p></div><div class="day-block"><b>After class</b><p>${escapeHTML(d.home||'—')}</p></div>${t?`<div class="day-block"><a class="back-link" href="#system/${Number(t.unit.split(' ')[1])}">Open unit guide →</a></div>`:''}</article>`}
...
function renderSystem(id){
  ...
  const vocabTotal=s.anatomyVocabulary.length+s.physiologyVocabulary.length;
  main.innerHTML=`<section class="system-hero" style="--unit-color:${s.color}"><div class="breadcrumbs"><a href="#systems">Body Systems</a> / Unit ${s.unit}</div><p class="overline">Unit ${s.unit}</p><h1>${escapeHTML(s.name)}</h1><p>${escapeHTML(s.kicker)}. Use this guide to connect what you must identify with what you must explain.</p><div class="system-stats"><div class="system-stat"><span>${s.topics.length}</span><small>Learning topics</small></div><div class="system-stat"><span>${vocabTotal}</span><small>Anatomy + physiology terms</small></div><div class="system-stat"><span>${new Set(s.topics.flatMap(t=>t.videos)).size}</span><small>Video alignments</small></div></div></section><div class="system-layout" style="--unit-color:${s.color}"><section><div class="section-heading"><div><p class="eyebrow" style="color:var(--unit-color)">Learning sequence</p><h2>What you’ll master</h2></div></div><div class="topic-list">${s.topics.map(topicItem).join('')}</div><div class="hero-actions">${prev?`<a class="button secondary" href="#system/${prev.unit}">← ${escapeHTML(prev.name)}</a>`:''}${next?`<a class="button" href="#system/${next.unit}">${escapeHTML(next.name)} →</a>`:''}</div></section><aside class="side-panel"><div class="vocab-card anatomy-vocab"><p class="eyebrow" style="color:var(--lime)">What to identify</p><h2>Anatomical structures</h2><p>Locate, label, and recognize these structures or landmarks.</p><div class="vocab-list">${s.anatomyVocabulary.map(v=>`<span class="vocab-chip">${escapeHTML(v)}</span>`).join('')}</div></div><div class="vocab-card physiology-vocab"><p class="eyebrow" style="color:var(--unit-color)">What to explain</p><h2>Physiology &amp; processes</h2><p>Use these terms to describe mechanisms, regulation, and change.</p><div class="vocab-list">${s.physiologyVocabulary.map(v=>`<span class="vocab-chip">${escapeHTML(v)}</span>`).join('')}</div></div><div class="connections-card"><p class="eyebrow" style="color:var(--unit-color)">Keep connecting</p><h2>Structure → function</h2><p>For every structure, ask: What feature allows it to do its job? What would happen if that feature changed?</p><a class="back-link" href="#calendar">View this week’s work →</a></div></aside></div>`;
}
...
search.oninput=()=>{const q=search.value.trim().toLowerCase();if(q.length<2){results.innerHTML='<p class="empty-state">Type at least two letters to search the full course.</p>';return}const found=[];DATA.systems.forEach(s=>{s.topics.forEach(t=>{if([t.title,t.target,t.detail,t.standard,...s.anatomyVocabulary,...s.physiologyVocabulary].join(' ').toLowerCase().includes(q))found.push({s,t})})});results.innerHTML=found.slice(0,30).map(({s,t})=>`<a class="search-result" href="#system/${s.unit}" data-close><b>${escapeHTML(t.code)} · ${escapeHTML(t.title)}</b><p>Unit ${s.unit}: ${escapeHTML(s.name)}</p></a>`).join('')||'<p class="empty-state">No matching course topics.</p>';results.querySelectorAll('[data-close]').forEach(a=>a.onclick=()=>dialog.close())};
